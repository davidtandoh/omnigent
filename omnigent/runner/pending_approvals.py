...
# Default wait budget for a UI verdict, in seconds. Held at INT_MAX
# (~68 years), effectively infinite: an ASK is a human-in-the-loop gate
# and must block until the human answers, never auto-refuse on its own.
# The old 120s default silently refused (treated as DENY) any prompt a
# user didn't answer within two minutes — the runner-side mirror of the
# cost-policy auto-resolve bug. Callers that resolve a per-policy
# ``ask_timeout`` should still pass ``timeout_seconds`` explicitly; this
# is only the fallback when none is provided. Headless/unattended agents
# that want a fast fail-closed should pass a finite ``timeout_seconds``.
_DEFAULT_WAIT_SECONDS: float = 2_147_483_647.0
